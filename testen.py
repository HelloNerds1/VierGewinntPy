...
class VierGewinnt():
    ...
    def Abfrage(self):
        ...
        while True:             # SR
            try:
                self.value = int(input('Spalte eingeben: ')) # Benutzereingabe in Ganzzahl
                if (input in ("0","1","2","3","4","5","6")):
                    break
                else:
                    print("passt nicht")
                    break   # Feld wird nur bei break ausgegeben
            except:
                continue # versuche es erneut, gehe zum Anfang der Schleife
            
            
    def SetMove(self, spieler):
                global counter 
                counter = counter +1

                if self.value < self.row_num:   # Hier wird gegengespüft, ob die eingegebene Zahl auch innerhalb des Spielfeldes liegt
                    if self.value == 0:
                        self.board[self.col0][self.value] = spieler   # hier wird das gewünschte Spieler-Symbol genutzt
                        self.col0 = self.col0 +1  
                    if self.value == 1:
                        self.board[self.col1][self.value] = spieler
                        self.col1 = self.col1 +1  
                    if self.value == 2:
                        self.board[self.col2][self.value] = spieler
                        self.col2 = self.col2 +1        
                    if self.value == 3:
                        self.board[self.col3][self.value] = spieler
                        self.col3 = self.col3 +1          
                    if self.value == 4:
                        self.board[self.col4][self.value] = spieler
                        self.col4 = self.col4 +1    
                    if self.value == 5:
                        self.board[self.col5][self.value] = spieler
                        self.col5 = self.col5 +1    
                    if self.value == 6:
                        self.board[self.col6][self.value] = spieler
        ...
        if modi == 1:
            ...
            while win != True:
                if (counter % 2) == 0:
                    ...
                    abfrage = vier_gewinnt.Abfrage()
                    setzten = vier_gewinnt.SetMove(player1.icon)
                    ausgeben = vier_gewinnt.Ausgabe() 
                    win = vier_gewinnt.CheckWin()
                elif (counter % 2) == 1:
                    player2.PlayerMessage()
                    abfrage = vier_gewinnt.Abfrage()
                    setzten = vier_gewinnt.SetMove(player2.icon)
                    ausgeben = vier_gewinnt.Ausgabe() 
                    win = vier_gewinnt.CheckWin()    
            break   
